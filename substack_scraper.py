...
class BaseSubstackScraper(ABC):
    ...
    @staticmethod
    def combine_metadata_and_content(url: str, author: str, title: str, subtitle: str, content: str) -> str:
        """
        Combines the title, subtitle, author, URL, and content into a single string in Markdown format.
        """
        if not all(isinstance(item, str) for item in [url, author, title, subtitle, content]):
            raise ValueError("All parameters must be strings")

        metadata_markdown = f"Title: {title}\n"
        if subtitle:
            metadata_markdown += f"Subtitle: {subtitle}\n"
        metadata_markdown += (
            f"Author: {author}\n"
            f"URL: {url}\n"
        ...
        return metadata_markdown + content


    def soup_to_md(self, url: str, soup: BeautifulSoup) -> str:
        """
        Converts Substack post soup to Markdown with structured metadata.
        """
        meta_author = soup.find('meta', attrs={'name': 'author'})
        author_name = meta_author['content'] if meta_author and 'content' in meta_author.attrs else "Unknown Author"
        ...
        content_md = self.html_to_md(content_html)

        return self.combine_metadata_and_content(url, author_name, title, subtitle, content_md)
